import asyncpg
from contextlib import asynccontextmanager


class Postgresql:

    def __init__(
            self,
            host: str,
            port: int,
            user: str,
            password: str,
            db: str
            ) -> None:
        self.host = host
        self.port = port
        self.user = user
        self.password = password
        self.db = db
        self.__pool: asyncpg.Pool | None = None

    @asynccontextmanager
    async def __create_connector(self):
        if self.__pool is None:
            self.__pool = await asyncpg.create_pool(
                    database=self.db,
                    user=self.user,
                    port=self.port,
                    password=self.password,
                    host=self.host)

        async with self.__pool.acquire() as connection:
            yield connection

    async def create(self, query: str, *args) -> None:
        async with self.__create_connector() as conn:
            async with conn.transaction():
                await conn.execute(query, *args)

    async def fetchrow(self, query: str, *args) -> dict:
        async with self.__create_connector() as conn:
            async with conn.transaction():
                resp = await conn.fetchrow(query, *args)
        return resp

    async def fetch(self, query: str, *args) -> dict:
        async with self.__create_connector() as conn:
            async with conn.transaction():
                resp = await conn.fetch(query, *args)
        return resp

    async def update(self, query: str, *args) -> None:
        async with self.__create_connector() as conn:
            async with conn.transaction():
                await conn.execute(query, *args)

    async def delete(self, query: str, *args) -> None:
        async with self.__create_connector() as conn:
            async with conn.transaction():
                await conn.execute(query, *args)
